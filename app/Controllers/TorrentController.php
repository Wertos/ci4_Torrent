...
class TorrentController extends BaseController
{
      ...
      public function TorrentView(?int $tId = null)
      {
            ...
            if ($torrentData->deleted_at != null) {
                  return redirect()
                        ->back()
                        ->with("error", lang("Torrent.deleted"));
            }
            ...
            if ($torrentData === null) {
                  return redirect()
                        ->back()
                        ->with("error", lang("Torrent.notfound"));
            }

            $torrentFile = $this->TorrentModel->torrLoad(
                  setting("Torrent.TorrentFilesPath"),
                  $torrentData->file_name,
            );
            ...
            if (!is_null($torrentFile)) {
                  ...
                  $filestree =
                        setting("Torrent.allowFileList") === true
                              ? $torrentFile->toTree()
                              : null;
            } else {
                  $annList = [];
            ...
            $data = [
                  "hash_v1" => $this->TorrentModel->hashToString(
                        $torrentData->infohash_v1,
                  ),
                  "hash_v2" => $this->TorrentModel->hashToString(
                        $torrentData->infohash_v2,
                  ),
                  ...
                  "moderate" => $this->isMod,
                  "download" =>
                        setting("Torrent.allowUploadTorrent") === true &&
                        in_array(
                              (int) $torrentData->modded,
                              setting("Torrent.statusAllowDownload"),
                        ) &&
                        $torrentFile,
                  "allowmagnet" =>
                  ...
                  "announceList" =>
                        count($annList) > 0 ? $annList : ["No tracker"],
            ...
            $siteTitle =
                  $this->TorrConfig->siteTitle . " | " . $torrentData->name;
      ...
      public function TorrentAddShow()
      {
            ...
            if (!$this->userData->can_upload) {
                  return redirect()
                        ->to("/")
                        ->with("error", lang("Torrent.uploadforbidden"));
            }

            $this->catList = $this->GlobalModel->getCats();

            $siteTitle =
                  $this->TorrConfig->siteTitle .
                  " | " .
                  lang("Torrent.addTorrent");
            ...
            $data = [
                  ...
                  "cats" => $this->catList,
                  "posterRequired" => setting("Torrent.posterRequired")
                        ? " required "
                        : "",
                  "smilies" => $table->generate($col_array),
            ];
      ...
      public function TorrentEditShow(int $tId)
      {
            ...
            if (!$can_edit) {
                  return redirect()
                        ->to("/")
                        ->with("error", lang("Torrent.notowner"));
            }
      ...
      public function TorrentEditAction(int $tId)
      {
            if (!$this->userData->can_upload) {
                  return redirect()
                        ->to("/")
                        ->with("error", lang("Torrent.editforbidden"));
            }
            ...
            $postData["url"] = url_title(
                  $this->translit->transliterate($postData["name"]),
                  "-",
                  true,
            );
            ...
            return redirect()
                  ->to("torrent/" . $tId)
                  ->with("message", lang("Torrent.editsuccess"));
      }
      ...
      public function TorrentAddAction()
      {
            ...
            $torrPath = $torrFile->store(
                  setting("Torrent.TorrentUploadPath"),
                  $torrName,
            );

            $this->torrent = $this->TorrentModel->torrLoad(
                  setting("Torrent.TorrentFilesPath"),
                  $torrName,
            );
            ...
            $arr = $this->TorrentModel->torrCheck(
                  $torrVersion,
                  isset($torrHashes[1]) ? $torrHashes[1] : null,
                  isset($torrHashes[2]) ? $torrHashes[2] : null,
            );
      ...
      public function TorrentSend($tId)
      {
            ...
            $torrentFile =
                  setting("Torrent.TorrentFilesPath") . $torrentData->file_name;
      ...
      public function TorrentDelete(int $tId)
      {
            ...
            if (!$can_delete) {
                  return redirect()
                        ->to("/")
                        ->with("error", lang("Torrent.deleteforbidden"));
            }
            ...
            return redirect()
                  ->to("/")
                  ->with("message", lang("Torrent.deletesucess"));
      }
}
