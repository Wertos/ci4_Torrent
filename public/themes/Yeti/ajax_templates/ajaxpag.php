  ...
  $metadata = '
                <ul class="list-inline home_metadata p-0 m-0">
                  ...
                  <li class="list-inline-item" title="'.lang('Torrent.downloaded').' '.number_format($torrent->downloaded).'"><i class="bi bi-eye"></i> '.number_format($torrent->views).'</li>
      					...
      					<div title="' . $status['title'] . '" class="border ' . $status['class'] . ' position-absolute top-0 start-100 translate-middle badge rounded-pill bg-secondary">' . $status['icon'] . ' '.getStrTorrVersion($torrent->version).'</div>
	';?>
