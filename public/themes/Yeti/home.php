...
<?php if($cat->count == 0) continue; ?>
  <div class="col-xs-12 col-sm-4 col-lg-6 mb-4">
   ...
   <!-- Default panel contents -->
   	<div class="card-header fs-6"><?= anchor($cat->url, $cat->name,['class' => 'link-primary text-decoration-none']); ?>
        ...
        $metadata = '
                <ul class="list-inline home_metadata p-0 m-0">
                  ...
                  <li class="list-inline-item" title="'.lang('Torrent.downloaded').' '.number_format($torrent->downloaded).'"><i class="bi bi-eye"></i> '.number_format($torrent->views).'</li>
      					...
      					<div class="border ' . $status['class'] . ' pt-1 pe-2 pb-1 ps-2 align-self-center position-absolute top-0 start-100 translate-middle badge rounded-pill bg-secondary">' . $status['icon'] . ' '.getStrTorrVersion($torrent->version).'</div>
			';?>
